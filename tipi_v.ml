...
let rec strongest_wizard (wizards : wizard list) : wizard option =
  ...
  |[] -> None
  |[x] -> Some x
  |x::y::tl -> if x.ability >= y.ability then strongest_wizard (x::tl)
  else strongest_wizard (y::tl)
...
*)
let rec max_list l f_max = 
	match l with 
	|[] -> None
	|[x] -> Some x
	|hd1 :: hd2::tl -> max_list ((f_max hd1 hd2)::tl) f_max
...
let vulnerable (spell:spell) (wizard :wizard) : vulnerability = effectiveness (school_of_spell spell) wizard.race
...
(* Vsak urok naredi toliko škode, kot je potrebnih točk mane za izvršitev uroka.
   ...
   int_of_float.
*)
let damage_caused spell wizard = 
...
*)
let attack (spell:spell) (tarca:wizard) : wizard = {tarca with hp = tarca.hp - damage_caused spell tarca}
...
let cast_spells (caster: wizard) : wizard * spell list = 
	...
	let rec iter m sez acc = 
		match (m,sez) with
		|(0,sez) -> ({caster with ability = m},List.rev acc)
		|(m,[]) -> ({caster with ability = m},List.rev acc)
		|(m,hd::tl) ->
			if (m >= (mana_of_spell hd))
			then iter (m-(mana_of_spell hd)) tl (hd::acc)
			else iter m tl acc 
	in
	iter m sez []	
...
let rec duel (attacker : wizard) (defender : wizard) : wizard = 
	let defender =
		if attacker.hp > 0
		then let (attacker,spells) = cast_spells attacker in
		match spells with
		|[] -> defender
		|_ -> 
