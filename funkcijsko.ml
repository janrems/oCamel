...
let mapi f l = 
	let rec mapi_aux f l acc n =
	match l with
	|[] -> acc
	|hd :: tl -> mapi_aux f tl ( (f n hd )::acc) (n +1)
	in reverse (mapi_aux f l [] 0)
...
let zip_enum_tlrec l1 l2 = 
	let rec zip_enum_tlrec_aux l1 l2 n acc =
		...
		|([],[])-> reverse acc
		|([],_)|(_,[]) -> failwith "Seznama razlicnih dolzin."
		|(hd1::tl1, hd2::tl2) -> zip_enum_tlrec_aux tl1 tl2 (n +1) ((n, hd1, hd2)::acc)
	in zip_enum_tlrec_aux l1 l2 0 []
