...
type 'a drevo = Rozno of 'a * 'a drevo list
...
let t = Rozno(1,[])
let t' = Rozno(2,[t;t])
let t'' = Rozno(3,[Rozno(-1,[]);t';Rozno(0,[])])
...
(*let izpisi_vrednosti t = 
	match Rozno(k,g) with 
	|Rozno(k,[]) -> print_int k
	|Rozno(k,g) -> print_int k; List.map izpisi_vrednosti g
*)
...
let rec globina t = 
	match t with 
	|Rozno(k,[]) -> 1
	|Rozno(k,g) -> 1+ List.fold_left (fun x y -> max x (globina y)) 0 g
...
let rec globoko_drevo  = function
	|1 -> Rozno(Random.int 5, [])
	|n -> Rozno(Random.int 5, [globoko_drevo (n-1)])
